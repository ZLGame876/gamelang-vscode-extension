...
for (const line of lines) {
    ...
    if (!trimmedLine || trimmedLine.startsWith('#')) {
        console.log(`[跳过] 第${lineNumber}行: ${trimmedLine || '(空行)'}`);
        continue;
    }
    
    console.log(`[执行] 第${lineNumber}行: ${trimmedLine}`);
    
    // 处理import语句
    if (trimmedLine.startsWith('import ')) {
        const moduleName = trimmedLine.replace('import ', '').replace(';', '').trim();
        console.log(`[导入] 成功导入模块: ${moduleName}`);
    }
    // 处理print语句
    else if (trimmedLine.match(/^(print|打印)\(/)) {
    ...
    // 处理函数定义
    else if (trimmedLine.match(/^fn\s+\w+\s*\([^)]*\)\s*:/) || trimmedLine.match(/^函数\s+\w+\s*\([^)]*\)\s*:/)) {
        console.log(`[函数定义] ${trimmedLine}`);
    }
    // 处理其他语句
    else {
...
console.log('\n=== 执行完成 ===');
console.log('提示: 这是简化版本，完整功能请使用F5键或命令面板运行'); 